...
const Navbar = ({ showBackButton = false, onBack }: NavbarProps) => {
  ...
  const scrollToSection = (e: React.MouseEvent<HTMLAnchorElement>, sectionId: string) => {
    e.preventDefault();
    const lenis = (window as unknown as { lenis?: { scrollTo: (target: HTMLElement | number, options: object) => void } }).lenis;
      ...
      <div className="flex gap-8 items-center">
        {!showBackButton && (
          <div className="hidden md:flex gap-8 text-[11px] uppercase font-bold tracking-[0.15em]">
            ...
            <Link href="/info" className="uppercase hover:text-coral transition-colors relative group">
              {t('navigation.info')}
              <span className="absolute -bottom-1 left-0 w-0 h-[1px] bg-ink-black transition-all duration-300 group-hover:w-full"></span>
            </Link>
          </div>
