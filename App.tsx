...
const ScrollProgress: React.FC = () => {
  ...
  return (
    ...
    >
      ...
      {/* Progress */}
      <motion.div
        className="absolute top-0 left-0 right-0 bg-black/60 rounded-full origin-top"
  ...
  return (
    <motion.span
      ...
      animate={{
        ...
        color: isHero ? '#2D2D2D' : 'transparent',
        WebkitTextStroke: isHero ? '1px transparent' : '1px #2D2D2D',
      }}
      transition={{
        opacity: {
          duration: 0.8,
  ...
  return (
    ...
    >
      ...
      <motion.div
        ...
        style={{
          background: 'radial-gradient(circle, rgba(45,45,45,0.04) 0%, transparent 60%)',
        }}
      ...
      <div className="absolute inset-0 overflow-hidden pointer-events-none">
        {[...Array(8)].map((_, i) => (
          <motion.div
            key={i}
            className="absolute w-0.5 h-0.5 bg-black/30 rounded-full"
            style={{
              left: `${10 + i * 12}%`,
              top: `${25 + (i % 4) * 15}%`,
            }}
      ...
      <div className="relative flex flex-col items-center mt-[25vh]">
        ...
        >
          <div className="relative h-[1px] bg-black/10 overflow-hidden">
            <motion.div
              className="absolute inset-y-0 left-0 bg-gradient-to-r from-black/60 via-black to-black/60"
              style={{ width: `${count}%` }}
            />
            <motion.div
              className="absolute inset-y-0 w-24 bg-gradient-to-r from-transparent via-black/20 to-transparent"
      ...
      <AnimatePresence mode="wait">
        {isSubmitted ? (
          ...
          <motion.button
            ...
            className="group flex items-center gap-2 font-mono text-[9px] text-ink-black/60 hover:text-ink-black transition-colors duration-300"
            initial={{ opacity: 0 }}
            animate={{ opacity: 1 }}
            exit={{ opacity: 0 }}
          >
...
const MainContent: React.FC<{ onOpenBooking: () => void }> = ({ onOpenBooking }) => {
  ...
  return (
    <motion.div
      ...
      animate={{ opacity: 1 }}
      transition={{ duration: 0.3 }}
      className="bg-paper-white min-h-[400vh] text-ink-black selection:bg-paper-white selection:text-paper-white font-sans antialiased relative cursor-none"
    >
      <Navbar />
      ...
      <div className="relative z-20 w-full flex flex-col gap-0">
        ...
        <section id="studios" className="bg-paper-white border-t border-black/10 relative">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 border-b border-black/10">
            ...
            {/* STUDIOS - Resident */}
            <div className="border-b md:border-b-0 md:border-r border-black/10 p-8 md:p-10">
              <motion.div
                initial={{ opacity: 0, y: 20 }}
                whileInView={{ opacity: 1, y: 0 }}
                viewport={{ once: true }}
              ...
              >
                ...
                <div className="space-y-6">
                  {[
                    ...
                    >
                      <h4 className="font-display text-xl md:text-2xl text-black/80 group-hover:text-ink-black transition-colors duration-300">
                        {item.studio}
                      </h4>
                      <div className="flex justify-between items-center mt-2">
                        ...
                        <span className={`font-mono text-[9px] uppercase tracking-[0.15em] ${
                          item.status === 'Current' ? 'text-sage/70' : 'text-black/25'
                        }`}>
                          {item.status}
                        </span>
                      </div>
                  ...
                  <div className="space-y-6 pl-6">
                    {[
                      ...
                      <motion.div
                        ...
                        whileHover={{ x: 4 }}
                      >
                        {/* Timeline dot - solid color to cover the line */}
                        <motion.div
                          className="absolute -left-6 top-2 w-2 h-2 rounded-full bg-paper-white border border-chrome group-hover:bg-chrome group-hover:border-ink-black transition-colors duration-300"
                          initial={{ scale: 0 }}
                          whileInView={{ scale: 1 }}
                          viewport={{ once: true }}
                        ...
                        <div className="flex items-baseline justify-between mb-1">
                          <h4 className="font-editorial italic text-xl md:text-2xl text-black/60 group-hover:text-ink-black transition-colors duration-300">
                            {item.city}
                          </h4>
                        ...
                        {/* Underline on hover */}
                        <motion.div
                          className="absolute -bottom-1 left-0 h-[1px] bg-black/20"
                          initial={{ width: 0 }}
            ...
            <div className="p-8 md:p-10">
              ...
              >
                  ...
                  <div className="space-y-6 pl-6">
                    {[
                      ...
                      <motion.div
                        ...
                        whileHover={{ x: 4 }}
                      >
                        {/* Timeline dot - solid colors to cover line */}
                        <motion.div
                          className={`absolute -left-6 top-2 w-2 h-2 rounded-full transition-colors duration-300 ${
                            item.isNext ? 'bg-coral border border-coral' : 'bg-paper-white border border-coral-light group-hover:bg-coral-light'
                          }`}
                          initial={{ scale: 0 }}
                          whileInView={{ scale: 1 }}
                          viewport={{ once: true }}
                        ...
                        {item.isNext && (
                          <motion.div
                            className="absolute -left-6 top-2 w-2 h-2 rounded-full bg-petal"
                        ...
                        <div className="flex items-baseline justify-between mb-1">
                          <div className="flex items-center gap-3">
                            <h4 className={`font-editorial italic text-xl md:text-2xl transition-colors duration-300 ${
                              item.isNext ? 'text-ink-black' : 'text-black/70 group-hover:text-ink-black'
                            }`}>
                              {item.city}
                            </h4>
                            {item.isNext && (
                        ...
                        <div className="overflow-hidden">
                          <motion.p
                            className="font-mono text-[9px] uppercase tracking-[0.2em] text-black/30 group-hover:text-black/50 transition-colors duration-300"
                            initial={{ y: 0 }}
                            whileHover={{ y: -2 }}
                          >
                            {item.studio}
                ...
                <motion.button
                  onClick={onOpenBooking}
                  className="mt-10 group relative flex items-center gap-3 px-5 py-3 border border-ink-black/40 hover:border-petal hover:bg-petal/20 rounded-full transition-all duration-300"
                  initial={{ opacity: 0 }}
                  whileInView={{ opacity: 1 }}
                  viewport={{ once: true }}
          ...
          <div className="grid grid-cols-1 md:grid-cols-2">
            {[
              ...
              <div key={idx} className="relative group border-b border-r border-black/10 h-[80vh] overflow-hidden">
                ...
                {/* Hover overlay with mix-blend + delayed blur animation */}
                <div className="absolute inset-0 flex flex-col justify-between p-8 opacity-0 group-hover:opacity-100 transition-opacity duration-500 delay-200">
                  <div className="flex justify-between items-start mix-blend-difference translate-y-[-10px] blur-sm group-hover:translate-y-0 group-hover:blur-0 transition-all duration-500 delay-300">
